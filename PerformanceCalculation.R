...
performance_calculation = function(x){
  DF = read_csv(x, col_names = FALSE) # load CSV
test_processing = DF %>% 
  ...
  mutate(times_appeared = as.numeric(ave(X1, X1, FUN = seq_along)) - 1) %>% # accumulating the times the an element has appeared before
